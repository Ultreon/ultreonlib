...
public final class ClientTicks {
    ...
    public static void scheduleAction(Runnable action) {
        ...
        if (INSTANCE.scheduledActions.size() >= QUEUE_OVERFLOW_LIMIT) {
            // Queue overflow?
            ModdingLibrary.LOGGER.warn("Too many client tick actions queued! Currently at " + INSTANCE.scheduledActions.size() + " items. Would have added '" + action + "'.",
                    new IllegalStateException("ClientTicks queue overflow"));
