...
public final class ServerTicks {
    ...
    public static void scheduleAction(Runnable action) {
        ...
        if (INSTANCE.scheduledActions.size() > QUEUE_OVERFLOW_LIMIT) {
            ModdingLib.LOGGER.warn("Too many server tick actions queued! Currently at {} items. Would have added '{}'.",
                    INSTANCE.scheduledActions.size(), action);
            ModdingLib.LOGGER.catching(new IllegalStateException("ServerTicks queue overflow"));
