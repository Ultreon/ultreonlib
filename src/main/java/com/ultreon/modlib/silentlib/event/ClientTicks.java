...
public final class ClientTicks {
    ...
    public static void scheduleAction(Runnable action) {
        if (GameUtil.isClient())
            ...
            ModdingLib.LOGGER.error("Tried to add client tick action on server side? {}", action);

        if (INSTANCE.scheduledActions.size() >= QUEUE_OVERFLOW_LIMIT) {
            // Queue overflow?
            ModdingLib.LOGGER.warn("Too many client tick actions queued! Currently at {} items. Would have added '{}'.",
                    INSTANCE.scheduledActions.size(), action);
            ModdingLib.LOGGER.catching(new IllegalStateException("ClientTicks queue overflow"));
