...
import com.ultreon.modlib.silentlib.util.GameUtil;
...
public final class ClientTicks {
    ...
    public static void scheduleAction(Runnable action) {
        if (GameUtil.isClient())
            ...
            UltreonModLib.LOGGER.error("Tried to add client tick action on server side? {}", action);

        if (INSTANCE.scheduledActions.size() >= QUEUE_OVERFLOW_LIMIT) {
            // Queue overflow?
            UltreonModLib.LOGGER.warn("Too many client tick actions queued! Currently at {} items. Would have added '{}'.",
                    INSTANCE.scheduledActions.size(), action);
            UltreonModLib.LOGGER.catching(new IllegalStateException("ClientTicks queue overflow"));
