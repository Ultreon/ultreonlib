...
public class OrderedMap<K, V> extends AbstractMap<K, V> {
    ...
    public List<Entry<K, V>> entryList() {
        synchronized (lock) {
            ...
            for (int i = 0; i < keys.size(); i++) {
                entries.add(new SimpleEntry<>(keys.get(i), values.get(i)));
            }
            return entries;
        }
    }
    ...
    public Entry<K, V> set(int index, K key, V value) {
        synchronized (lock) {
            ...
            if (oldKeyIndex == index) {
                ...
                return new SimpleEntry<>(oldKey, oldValue);
            }
    ...
    private Entry<K, V> getFirst() {
        synchronized (lock) {
            ...
            return new SimpleEntry<>(keys.get(0), values.get(0));
        }
    }
    ...
    public Entry<K, V> getLast() {
        synchronized (lock) {
            ...
            return new SimpleEntry<>(keys.get(keys.size() - 1), values.get(values.size() - 1));
        }
    }
