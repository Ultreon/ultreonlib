...
abstract sealed class McWindowManager extends McComponent permits McOperatingSystem {
    final Deque<McWindow> windows = new ArrayDeque<>();
    private final Deque<McWindow> midStack = new ArrayDeque<>();
    private final Deque<McWindow> topStack = new ArrayDeque<>();
    private final Deque<McWindow> bottomStack = new ArrayDeque<>();
    private final Object wmLock = new Object();
    private McWindow pressedWindow;

    private McWindow activeWindow;
    protected int windowActiveColor = 0xffdc143c;
    protected int windowInactiveColor = 0xff555555;
    Insets border;
    private final McOperatingSystem system;
    ...
    public McWindowManager(int x, int y, int width, int height, List<McWindow> windows) {
        ...
        if (this instanceof McOperatingSystem operatingSystem) {
            this.system = operatingSystem;
        } else {
            this.system = McOperatingSystem.get();
        }
    ...
    private void renderWindow(@NotNull GuiGraphics gfx, int mouseX, int mouseY, float partialTicks, @NotNull McWindow window, McWindow hoveredWindow) {
        @NotNull McWindow _window = window;
        @Nullable McDialogWindow dialog = _window.getDialog();
    ...
    synchronized void setTopMost(McWindow window, boolean topMost) {
        synchronized (this.wmLock) {
    ...
    synchronized void setBottomMost(McWindow window, boolean bottomMost) {
        synchronized (this.wmLock) {
    ...
    synchronized void destroyWindow(McWindow window) {
        synchronized (this.wmLock) {
            try {
                ...
                this.system.getLogger().error("Application crash:", e);
                this.system.destroyApplication(window.application);
                return;
            }
            ...
            setNewWindow: {
                ...
                try {
                    ...
                    this.system.getLogger().error("Application crash:", e);
                    this.system.destroyApplication(newWindow.application);
                }
            }
        }
    }

    synchronized void moveToForeground(McWindow window) {
        synchronized (wmLock) {
            try {
                ...
                this.system.getLogger().error("Application crash:", e);
                this.system.destroyApplication(this.pressedWindow.application);
                return;
            }
    ...
    public boolean isValid(McWindow window) {
